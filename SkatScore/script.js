document.addEventListener('DOMContentLoaded', () => {
    ...
    optionenInputs.forEach((input, i) => {
      input.addEventListener('change', () => {
        if (input.checked) {
          // Wenn diese Option angewählt wird:
          // Alle vorherigen + diese anwählen
          for (let j = 0; j <= i; j++) {
            optionenInputs[j].checked = true;
          }
          // Alle nachfolgenden abwählen
          for (let k = i+1; k < optionenInputs.length; k++) {
            optionenInputs[k].checked = false;
          }
        } else {
          // Wenn diese Option abgewählt wird:
          // Diese ist bereits abgewählt, alle folgenden ebenfalls abwählen
          for (let k = i+1; k < optionenInputs.length; k++) {
            optionenInputs[k].checked = false;
          }
          // Die vorherigen Optionen bleiben unverändert.
        }
      });
    });
  ...
  spielArtRadios.forEach(radio => {
    ...
    form.addEventListener('submit', function(event) {
      ...
      form.reset(); // Bei Bedarf auskommentieren, um Auswahl zu behalten
    });
  });
